...
// Size at which to ACK
#define ACK_SIZE (16*1024L*1024L)
...
int run_send_test(int msgsize)
{
    ...
    while (now <= start + TOTAL_TIME*1000000) {
    	...
    	error = socket_send(socketfd, buf, msgsize, PACKET_SIZE, true);
    	...
    	error = socket_receive(socketfd, buf, 1, 1, false);
...
int run_receive_test(int msgsize)
{
    ...
    while (buf[0] == 0) {
    	...
    	error = socket_receive(socketfd, buf,  msgsize, PACKET_SIZE, true);
