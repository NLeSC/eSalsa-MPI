...
public class VirtualConnection {
    ...
    // Fragment a server message
    public byte [][] fragmentMessage(ServerMessage m) throws IOException {
        return out.fragmentMessage(m);
    }
    
    // Defragment a server message
    public CommunicatorRequest receivedMessage(byte [] message, int length) throws IOException {
        return (CommunicatorRequest) in.addMessage(message, length);
    }
}
