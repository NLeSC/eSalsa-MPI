...
//INFO(1, "xEMPI_Barrier WA BARRIER", "LOCAL BARRIER(2)");
   ...
   if (error != EMPI_SUCCESS) {
      ERROR(1, "xEMPI_Barrier: local barrier failed! (2) (comm=%d, error=%d)", c->handle, error);
      return EMPI_ERR_INTERN;
   }
...
int xEMPI_Allgatherv(void *sendbuf, int sendcount, EMPI_Datatype sendtype,
...
int xEMPI_Scatter(void* sendbuf, int sendcount, EMPI_Datatype sendtype,
                 void* recvbuf, int recvcount, EMPI_Datatype recvtype,
                 int root, EMPI_Comm comm)
{
   int i, error;
...
//INFO(1, "local scatter");
     ...
     return TRANSLATE_ERROR(MPI_Scatter(sendbuf, sendcount, stype->type, recvbuf, recvcount, rtype->type, root, c->comm));
   }
...
int xEMPI_Scatterv(void* sendbuf, int *sendcounts, int *displs, EMPI_Datatype sendtype,
                  void* recvbuf, int recvcount, EMPI_Datatype recvtype,
                  int root, EMPI_Comm comm)
{
   communicator *c;
   ...
   if (comm_is_local(c)) {
     ...
     return TRANSLATE_ERROR(MPI_Scatterv(sendbuf, sendcounts, displs, stype->type, recvbuf, recvcount, rtype->type, root, c->comm));
   }
...
int xEMPI_Reduce(void* sendbuf, void* recvbuf, int count, EMPI_Datatype type, EMPI_Op op, int root, EMPI_Comm comm)
{
...
// FIXME: use size own type info ???
      ...
      error = TRANSLATE_ERROR(MPI_Type_extent(t->type, &extent));
...
INFO(2, "Starting local reduce sendbuf=%p buffer=%p count=%d root=%d get_local_rank(root)=%d",
   ...
   error = TRANSLATE_ERROR(MPI_Reduce(sendbuf, buffer, count, t->type, o->op, get_local_rank(c, local_root), c->comm));
...
int xEMPI_Accumulate (void *origin_addr, int origin_count, EMPI_Datatype origin_datatype,
                     int target_rank, MPI_Aint target_disp, int target_count, EMPI_Datatype target_datatype,
                     EMPI_Op op, EMPI_Win win)
{
   ERROR(1, "Operation not implemented: xEMPI_Accumulate");
   return EMPI_ERR_INTERN;
/*
                         ...
                         EMPI_Datatype type, EMPI_Op op, EMPI_Comm comm)
{
   ...
   if (comm_is_local(c)) {
     ...
     return TRANSLATE_ERROR(MPI_Allreduce(sendbuf, recvbuf, count, t->type, o->op, c->comm));
   }
...
//                       c->global_rank, c->local_rank, count, ((int *)sendbuf)[1], ((int *)recvbuf)[1]);
   ...
   error = TRANSLATE_ERROR(MPI_Reduce(sendbuf, recvbuf, count, t->type, o->op, get_local_rank(c, c->my_coordinator), c->comm));
...
//   INFO(1, "LOCAL BAST grank=%d lrank=%d count=%d buf[1]=%d\n", c->global_rank, c->local_rank, count, ((int*)recvbuf)[1]);
   ...
   return TRANSLATE_ERROR(MPI_Bcast(recvbuf, count, t->type, 0, c->comm));
}
              ...
              EMPI_Datatype datatype, EMPI_Op op, EMPI_Comm comm)
{
   ...
   if (comm_is_local(c)) {
     ...
     return TRANSLATE_ERROR(MPI_Scan(sendbuf, recvbuf, count, t->type, o->op, c->comm));
   }
                        ...
                        communicator *c)
{
   ...
   // First retrieve the data element size
   // FIXME: use size?
   error = TRANSLATE_ERROR(MPI_Type_extent(sendtype->type, &sextent));
   ...
   // First retrieve the data element size
   // FIXME: use size?
   error = TRANSLATE_ERROR(MPI_Type_extent(recvtype->type, &rextent));
...
int xEMPI_Alltoall(void *sendbuf, int sendcount, EMPI_Datatype sendtype,
                  void *recvbuf, int recvcount, EMPI_Datatype recvtype,
                  EMPI_Comm comm)
{
   int i, error;
   ...
   if (comm_is_local(c)) {
     ...
     return TRANSLATE_ERROR(MPI_Alltoall(sendbuf, sendcount, stype->type, recvbuf, recvcount, rtype->type, c->comm));
   }
...
int xEMPI_Alltoallv(void *sendbuf, int *sendcounts, int *sdispls, EMPI_Datatype sendtype,
                    ...
                    EMPI_Comm comm)
{
   ...
   if (comm_is_local(c)) {
     // simply perform an all-to-all in local cluster
     return TRANSLATE_ERROR(MPI_Alltoallv(sendbuf, sendcounts, sdispls, stype->type, recvbuf, recvcounts, rdispls, rtype->type, c->comm));
   }
...
int xEMPI_Comm_dup(EMPI_Comm comm, EMPI_Comm *newcomm)
{
   ...
   error = TRANSLATE_ERROR(MPI_Comm_dup(c->comm, &tmp_com));
...
// FIXME: is this correct ?????
   ...
   error = TRANSLATE_ERROR(MPI_Comm_group(world->comm, &orig_group));
   ...
   error = TRANSLATE_ERROR(MPI_Group_incl(orig_group, local_count, local_members, &new_group));
   ...
   error = TRANSLATE_ERROR(MPI_Comm_create(c->comm, new_group, newcomm));
...
int xEMPI_Comm_create(EMPI_Comm mc, EMPI_Group mg, EMPI_Comm *newcomm)
{
...
int xEMPI_Comm_split(EMPI_Comm comm, int color, int key, EMPI_Comm *newcomm)
{
   ...
   error = TRANSLATE_ERROR(MPI_Comm_split(c->comm, reply.color, reply.key, &tmp));
   ...
   STACKTRACE(0, "COMM_SPLIT %d -> %d", c->handle, result->number);
...
int xEMPI_Group_rank(EMPI_Group mg, int *rank)
{
   group *g = handle_to_group(mg);
...
int xEMPI_Group_size(EMPI_Group mg, int *size)
{
   group *g = handle_to_group(mg);
...
int xEMPI_Group_incl(EMPI_Group mg, int n, int *ranks, EMPI_Group *newgroup)
{
   group *res;
   group *g;
...
int xEMPI_Group_range_incl(EMPI_Group mg, int n, int ranges[][3], EMPI_Group *newgroup)
{
   group *res;
   group *g;
...
int xEMPI_Group_translate_ranks(EMPI_Group group1, int n, int *ranks1,
